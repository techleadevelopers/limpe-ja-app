...
// Importa AuthResponse e UserRole do seu arquivo de tipos centralizado
import { AuthResponse, UserRole } from '../types/backend/auth';
import { UserProfile } from '../types/backend/users'; // Certifique-se de que UserProfile está completa aqui
...
class AuthService {
    ...
    async login(credentials: { email: string; password: string }): Promise<AuthResponse> {
        try {
            ...
            const authData: AuthResponse = response.data; // Agora AuthResponse é o tipo importado
    ...
    private async saveAuthData(authData: AuthResponse): Promise<void> {
        try {
            ...
            await AsyncStorage.setItem(AUTH_TOKEN_KEY, authData.accessToken); // accessToken
            await AsyncStorage.setItem(USER_ROLE_KEY, authData.user.role); // role está dentro de user
            await AsyncStorage.setItem(USER_ID_KEY, authData.user.id); // id está dentro de user
