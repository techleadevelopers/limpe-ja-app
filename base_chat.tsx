...
export default function ChatScreen() {
  ...
  useEffect(() => {
    ...
    const loadChatData = async () => {
      ...
      try {
        ...
        if (bookingId) {
          ...
          if (bookingDetails.status === BookingStatus.FINISHED) {
            ...
            setChatBlockedMessage(
              `FaĂ§a o pagamento do serviĂ§o para falar com ${recipientName || bookingDetails.providerFullName || 'o prestador'}.`
            );
          }
        }
      } catch (error: any) {
        ...
        if (
          error.message.includes('NĂŁo Ă© possĂ­vel acessar esta conversa') ||
          error.message.includes('NĂŁo Ă© possĂ­vel enviar mensagens')
        ) {
          setChatBlockedMessage(normalized);
        } else {
          Alert.alert('Erro', 'NĂŁo foi possĂ­vel carregar as mensagens do chat.');
          setChatBlockedMessage('NĂŁo foi possĂ­vel carregar as mensagens.');
        }
      } finally {
        setIsLoading(false);
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    try {
      ...
      const normalized = getUserMessage(error);
      if (error.message.includes('NĂŁo Ă© possĂ­vel enviar mensagens')) {
        setChatBlockedMessage(normalized);
      } else {
