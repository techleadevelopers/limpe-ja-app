...
import { RolesGuard } from '../auth/guards/roles.guard'; // Assumindo que vocÃª tem este guard
import { Roles } from '../auth/decorators/roles.decorator'; // Assumindo que vocÃª tem este decorator
import { UserRole } from '@prisma/client'; // Assumindo que UserRole estÃ¡ no seu schema Prisma
...
export class SupportController {
  ...
  async createTicket(@Request() req, @Body() createTicketDto: CreateTicketDto) {
    // CORREÃ‡ÃƒO: Alterado req.user.id para req.user.userId
    const userId = req.user.userId;
    const userRole = req.user.role; // Assumindo que a role estÃ¡ no token JWT
  ...
  async getTickets(
    ...
    const userId = req.user.userId; // Ajuste similar pode ser necessÃ¡rio aqui se este endpoint for usado por nÃ£o-admins
  ...
  @Get('tickets/:id')
  @ApiOperation({ summary: 'ObtÃ©m detalhes de um ticket de suporte' })
  async getTicketDetails(@Request() req, @Param('id') ticketId: string) {
    const userId = req.user.userId; // Ajuste similar pode ser necessÃ¡rio aqui
    const userRole = req.user.role;
    ...
    // Garante que apenas o proprietÃ¡rio ou um admin pode ver o ticket
    if (ticket.userId !== userId && userRole !== UserRole.ADMIN) {
      throw new NotFoundException('Ticket nÃ£o encontrado ou sem permissÃ£o.'); // Usar NotFound para nÃ£o vazar info
    }
