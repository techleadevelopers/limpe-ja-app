...
async function bootstrap() {
  ...
  app.enableCors({
    origin: [
      ...
      // Para o app nativo (iOS/Android), CORS não é um problema, mas para o Expo Web sim.
    ],
  ...
  try {
    ...
    if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
        ...
        // Aviso se o SDK não foi inicializado de forma alguma Firebase Admin SDK não foi inicializado. Funções que dependem dele podem falhar.');]
        console.warn('[Firebase Admin] Firebase Admin SDK não foi inicializado. Funções que dependem dele podem falhar.');
        // Opcional: process.exit(1); para forçar o encerramento do app se o Firebase Admin for crítico
    }
  }
