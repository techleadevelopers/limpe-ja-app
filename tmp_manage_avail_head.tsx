...
// DefiniĂ§ĂŁo manual da interface Theme (baseada na doc oficial da lib)
interface Theme {
...
// ImportaĂ§Ăµes de serviĂ§os e tipos do backend
import {
...
LocaleConfig.locales['pt-br'] = {
  ...
  dayNames: ['Domingo', 'Segunda-feira', 'TerĂ§a-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'SĂˇbado'],
  dayNamesShort: ['DOM', 'SEG', 'TER', 'QUA', 'QUI', 'SEX', 'SĂB'],
  today: 'Hoje'
};
  ...
  return (
    <Animated.View style={{ transform: [{ scale: animatedScale }] }}>
      <TouchableOpacity
        ...
        accessibilityLabel={`HorĂˇrio ${time}${isSelected ? ' selecionado' : (isBooked || isDisabled) ? ' indisponĂ­vel' : ''}`}
    ...
    <Animated.View style={[styles.dayCard, { opacity: cardAnim, transform: [{ translateY: cardAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }]}>
      <View style={styles.dayHeader}>
        ...
        <TouchableOpacity
          ...
          accessibilityRole="button"
          accessibilityLabel={showFullGrid ? 'Concluir personalizaĂ§ĂŁo' : 'Personalizar horĂˇrios'}
        >
      ...
      <View style={styles.periodRow}>
        <TouchableOpacity
          ...
          disabled={isPastDay}
          accessibilityRole="button"
          accessibilityLabel={`Definir manhĂŁ em ${dayName}`}
        >
          <Ionicons name="sunny-outline" size={18} color={currentPeriod === 'morning' ? '#fff' : Colors.primary} />
          <Text style={[styles.periodTileText, currentPeriod === 'morning' && styles.periodTileTextActive]}>ManhĂŁ</Text>
          <Text style={[styles.periodTileSub, currentPeriod === 'morning' && styles.periodTileTextActive]}>08â€“12</Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          ...
          <Text style={[styles.periodTileSub, currentPeriod === 'afternoon' && styles.periodTileTextActive]}>13â€“17</Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          ...
          <Text style={[styles.periodTileSub, currentPeriod === 'evening' && styles.periodTileTextActive]}>18â€“21</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.personalizeButton}
          onPress={() => setShowFullGrid((v) => !v)}
          accessibilityRole="button"
          accessibilityLabel="Personalizar horĂˇrios"
        >
      ...
      {/* CorreĂ§ĂŁo aqui: Adicionei flexWrap, gap e justifyContent para responsividade */}
      <View style={[styles.quickActionsRow, { flexWrap: 'wrap', gap: 4, justifyContent: 'space-around' }]}>
        {/* Sobrescrevi width para 30% apenas nesses botĂµes (corrige o overflow do "Dia todo") */}
        <TouchableOpacity
          style={[styles.quickTile, { width: '30%' }]}
          onPress={() => !isPastDay && onApplyPreset(dayOfWeek, 'morning')}
          disabled={isPastDay}
          accessibilityRole="button"
          accessibilityLabel={`Aplicar manhĂŁ em ${dayName}`}
        >
          <Text style={styles.quickActionText}>ManhĂŁ</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.quickTile, { width: '30%' }]}
          onPress={() => !isPastDay && onApplyPreset(dayOfWeek, 'afternoon')}
        ...
        <View>
          ...
          <View style={styles.dayActions}>
            {/* <TouchableOpacity 
              ...
              accessibilityRole="button" 
              accessibilityLabel="Selecionar todos os horĂˇrios"
            >
            ...
            <TouchableOpacity 
              ...
              accessibilityRole="button" 
              accessibilityLabel="Limpar horĂˇrios selecionados"
            >
            ...
            <TouchableOpacity 
              ...
              accessibilityRole="button" 
              accessibilityLabel={`Reverter alteraĂ§Ăµes em ${dayName}`}
            >
...
export default function ManageAvailabilityScreen() {
  ...
  const saveRadius = useCallback(async () => {
    try {
      ...
      // sinaliza para a tela Explore recarregar recomendaĂ§Ăµes
  ...
  const dayNames = ['Domingo', 'Segunda-feira', 'TerĂ§a-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'SĂˇbado'];

  // ===== Handlers movidos para cima (apĂłs states, antes de useEffects/JSX) =====
  ...
  // handleToggleDay (convertido para function declaration - hoisted, sem useCallback desnecessĂˇrio)
  ...
  function handleDayPressOnCalendar(day: DateData) {
    ...
    if (day.dateString < todayStr) {
      Alert.alert('Data invĂˇlida', 'Selecione apenas hoje ou datas futuras.');
      AccessibilityInfo.announceForAccessibility?.('Data invĂˇlida. Selecione apenas hoje ou datas futuras.');
      return;
    }
    ...
    AccessibilityInfo.announceForAccessibility(`Data selecionada: ${day.dateString}. Configure exceĂ§ĂŁo se desejar.`);
  }
  ...
  const loadData = useCallback(async () => {
    if (!user?.id) {
      console.warn("User ID nĂŁo disponĂ­vel, nĂŁo carregando dados de disponibilidade.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      Alert.alert('Erro', 'NĂŁo foi possĂ­vel carregar sua disponibilidade. Tente novamente.');
    } finally {
      setIsLoading(false);
    }
  ...
  const applyCopyToTargets = useCallback((overrideFrom?: number, overrideTargets?: number[]) => {
    ...
    setWeeklyAvailability(prev => prev.map(d => {
      ...
      // NĂŁo copiar para dias passados
  ...
  const applySmartStep = useCallback(() => {
    try {
      if (smartMode === 'weekly') {
        ...
        // Bloqueia datas passadas tambĂ©m no fluxo do assistente
        const todayStr = new Date().toISOString().split('T')[0];
        if (smartDate < todayStr) {
          AccessibilityInfo.announceForAccessibility?.('A data escolhida jĂˇ passou. Selecione hoje ou futura.');
          return;
        }
        ...
        setSmartVisible(false);
        if (Platform.OS === 'ios') Haptics.selectionAsync();
        AccessibilityInfo.announceForAccessibility?.('ExceĂ§ĂŁo atualizada');
      }
    } catch (e) {
  ...
  const handleClearOverride = useCallback(() => {
    ...
    if (Platform.OS === 'ios') Haptics.selectionAsync();
    AccessibilityInfo.announceForAccessibility('ExceĂ§ĂŁo removida para esta data.');
  }, [selectedDateForOverride]);

  const handleSaveAvailability = useCallback(async () => {
    if (!user?.id) {
      Alert.alert("Erro", "ID do provedor nĂŁo encontrado. FaĂ§a login novamente.");
      return;
    }
    ...
    try {
      ...
      Alert.alert('Erro', error.response?.data?.message || 'NĂŁo foi possĂ­vel salvar sua disponibilidade. Tente novamente.');
    } finally {
      setIsSaving(false);
      loadData();
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ backgroundColor: Colors.surface, borderRadius: Radii.sm, padding: Spacing.md, marginHorizontal: Spacing.sm, marginBottom: Spacing.md, borderWidth: 1, borderColor: Colors.border }}>
        ...
        >
          <Text style={[styles.segmentText, activeTab === 'overrides' && styles.segmentTextActive]}>ExceĂ§Ăµes</Text>
        </TouchableOpacity>
      </View>
      ...
      >
        {activeTab === 'weekly' && (
          <>
            {false && <View style={styles.quickTilesRow}>
              ...
              >
                <Text style={styles.quickActionText}>DisponĂ­vel hoje 08-12</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={styles.quickTile} 
                onPress={() => {
                  ...
                  if (Platform.OS === 'ios') Haptics.selectionAsync();
                  AccessibilityInfo.announceForAccessibility('Disponibilidade salva para amanhĂŁ 13-17');
                }}
              >
                <Text style={styles.quickActionText}>DisponĂ­vel amanhĂŁ 13-17</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={styles.quickTile} 
                onPress={() => {
            ...
            {false && (<TouchableOpacity 
              ...
              accessibilityRole="button" 
              accessibilityLabel="Assistente de HorĂˇrios"
            >
              <Ionicons name="flash-outline" size={18} color="#fff" style={{ marginRight: 8 }} accessibilityHidden={true} />
              <Text style={styles.primaryCTAButtonText}>Assistente de HorĂˇrios</Text>
            </TouchableOpacity>)}
            <Text style={styles.sectionTitleImproved}>Disponibilidade Semanal</Text>
            <InfoCard text="Defina seus horĂˇrios da semana. VocĂŞ pode usar horĂˇrios prontos e copiar para os outros dias." />
            {([...weeklyAvailability]
        ...
        {activeTab === 'overrides' && (
          <>
            <Text style={styles.sectionTitleImproved}>ExceĂ§Ăµes de Datas</Text>
            <TouchableOpacity 
              style={styles.primaryCTAButton} 
              onPress={() => openSmartStep('override')} 
              accessibilityRole="button" 
              accessibilityLabel="Criar exceĂ§ĂŁo rĂˇpida"
            >
              <Ionicons name="calendar-outline" size={18} color="#fff" style={{ marginRight: 8 }} accessibilityHidden={true} />
              <Text style={styles.primaryCTAButtonText}>Criar exceĂ§ĂŁo rĂˇpida</Text>
            </TouchableOpacity>
            <InfoCard text="Selecione uma data no calendĂˇrio para bloquear ou definir horĂˇrios personalizados." />
            <View style={styles.calendarOverrideContainer}>
              <Calendar
                ...
                accessibilityLabel="CalendĂˇrio para exceĂ§Ăµes de datas"
                accessibilityHint="Selecione uma data para configurar exceĂ§ĂŁo"
              />
              {selectedDateForOverride && (
                <View style={styles.overrideOptionsCard}>
                  <Text style={styles.overrideTitle}>OpĂ§Ăµes para {selectedDateForOverride}</Text>
                  ...
                  {currentOverride?.type === 'custom' && currentOverride.selectedSlots && currentOverride.selectedSlots.length > 0 && (
                    <Text style={styles.customSlotsSummary}>
                      HorĂˇrios selecionados: {currentOverride.selectedSlots.slice(0, 3).join(', ')}
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.92}
                    accessibilityRole="button"
                    accessibilityLabel="Definir horĂˇrios personalizados"
                  >
                    <Text style={[styles.overrideButtonText, currentOverride?.type === 'custom' && styles.overrideButtonTextSelected]}>Definir HorĂˇrios Personalizados</Text>
                  </TouchableOpacity>
                  ...
                  {currentOverride && (
                    <TouchableOpacity 
                      ...
                      activeOpacity={0.92} 
                      accessibilityRole="button" 
                      accessibilityLabel="Remover exceĂ§ĂŁo"
                    >
                      <Text style={styles.clearOverrideButtonText}>Remover ExceĂ§ĂŁo para este Dia</Text>
                    </TouchableOpacity>
                  )}
                </View>
              )}
      ...
      <View style={styles.stickySaveBar} accessibilityRole="summary">
        <Text style={styles.stickySummaryText}>
          {pendingChangesCount > 0 ? `${pendingChangesCount} alteraĂ§Ăµes pendentes` : 'Tudo salvo'}
        </Text>
        <TouchableOpacity
          ...
          activeOpacity={0.92}
          accessibilityRole="button"
          accessibilityLabel="Salvar alteraĂ§Ăµes"
        >
          {isSaving ? (
      ...
      {smartVisible && (
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{smartMode === 'weekly' ? 'Assistente de HorĂˇrios' : 'ExceĂ§ĂŁo RĂˇpida'}</Text>
                ...
                {smartMode === 'weekly' ? (
                  <View style={styles.modalChipsRow}>
                    ...
                    <TouchableOpacity 
                      ...
                      accessibilityRole="button"
                    >
                      <Text style={[styles.modalChipText, smartScope === 'tomorrow' && styles.modalChipTextActive]}>AmanhĂŁ</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                    ...
                    <TouchableOpacity 
                      ...
                      accessibilityRole="button"
                    >
                      <Text style={[styles.modalChipText, smartScope === 'date' && styles.modalChipTextActive]}>Data no calendĂˇrio</Text>
                    </TouchableOpacity>
                  </View>
                ) : (
            ...
            {smartStep === 2 && (
              <View>
                <Text style={styles.stepTitle}>{smartMode === 'weekly' ? 'Qual horĂˇrio?' : 'Bloquear ou personalizar?'}</Text>
                {smartMode === 'weekly' ? (
                  <View style={styles.modalChipsRow}>
                    <TouchableOpacity 
                      ...
                      accessibilityRole="button"
                    >
                      <Text style={[styles.modalChipText, smartPreset === 'morning' && styles.modalChipTextActive]}>ManhĂŁ (08-12)</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
              ...
              <View>
                ...
                <View style={styles.summaryCard}>
                  ...
                  <Text style={{ flex: 1, color: Colors.text }}>
                    {smartMode === 'weekly' ? (
                      smartScope === 'today' ? 'Hoje' : smartScope === 'tomorrow' ? 'AmanhĂŁ' : smartScope === 'weekday' ? `Dias: ${smartWeekdays.map(d => dayNames[d].slice(0, 3)).join(', ')}` : 'Data no calendĂˇrio'
                    ) : (
                    ...
                    {(() => {
                      ...
                      return smartCustomSlots.length > 0 ? `${smartCustomSlots.length} horĂˇrios` : (smartMode === 'override' && smartOverrideType === 'blocked' ? 'Bloqueado' : 'Personalizar');
                    })()}
                  </Text>
                </View>
                <View style={styles.modalActions}>
...
const styles = StyleSheet.create({
  ...
  periodTile: {
    ...
    borderWidth: 0.5,
    borderColor: Colors.border,
    // 3 colunas fixas no topo: ManhĂŁ, Tarde, Noite
    width: '32%',
    minWidth: 96,
    alignItems: 'center',
  ...
  personalizeButton: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    // Escondido (botĂŁo movido para o cabeĂ§alho)
    display: 'none',
  },
