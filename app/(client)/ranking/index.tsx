...
import RankingService from '../../../services/rankingService'; // <- corrigido
...
export default function RankingScreen() {
      ...
      <View style={[styles.avatarWrap, { width: size + 8, height: size + 8, borderRadius: (size + 8) / 2, backgroundColor: withAlpha('#000', 0.08) }]}>
        <View
          style={{
            ...
            borderWidth: 2,
            borderColor: crown ? theme.link : theme.background,
          }}
        >
          <Ionicons name="person-circle-outline" size={size - 4} color={theme.grey} />
        </View>
      </View>
      {crown && (
        // Ícone 3D (coroa) no lugar do trophy linear
        <Icon3D src={Icons3D.crown} size={26} style={{ marginTop: -8, marginBottom: 4 }} />
      )}
      <Text style={[styles.podiumScore, { color: '#FFF' }]}>{e.score}</Text>
      <Text style={[styles.podiumHandle, { color: withAlpha('#FFF', 0.8) }]}>{e.handle || e.displayName}</Text>
    </View>
  );
    ...
    <View style={styles.tabs}>
      {[
        ...
        return (
          ...
          >
            <Text style={[styles.tabText, { color: '#FFF', fontWeight: active ? '800' : '600' }]}>{t.label}</Text>
          </TouchableOpacity>
        );
      })}
    </View>
  ...
  const HeaderHero = () => (
    ...
    >
      <LinearGradient colors={['#6E34E8', '#9F2CF3']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.heroGrad}>
        ...
        <View style={styles.myRankBar}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            <Text style={styles.myRankText}>
              Você está em <Text style={{ fontWeight: '900' }}>#{myRank.rank}</Text> com {myRank.score} pts
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: '#FFF', fontWeight: '800', letterSpacing: 0.6 },

  hero: { margin: 16, marginBottom: 10, borderRadius: 24, overflow: 'hidden' },
  heroGrad: { padding: 16, borderRadius: 24 },
  heroTitle: { color: '#FFF', fontSize: 12, fontWeight: '800', letterSpacing: 1.2, opacity: 0.9 },
  ...
  podiumScore: { marginTop: 6, fontSize: 20, fontWeight: '800' },
  podiumHandle: { fontSize: 12, marginTop: 2 },
