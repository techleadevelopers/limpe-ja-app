...
import * as Haptics from 'expo-haptics'; // ✅ NOVO: Import para haptics em micro-interações
import { LinearGradient } from 'expo-linear-gradient';
import { Stack, useLocalSearchParams, useRouter, useFocusEffect } from 'expo-router'; // ✅ NOVO: useFocusEffect para pausar animações
...
import {
  ...
  ImageBackground,
  ScrollView,
  AppState, // ✅ NOVO: Para detectar perda de foco e pausar loops
} from 'react-native';
...
const BookingSummaryPreview = ({
  ...
  // ✅ ACESSIBILIDADE: Adicionar accessibilityLabel ao resumo flutuante (mas aqui é o preview; aplicar similar no floating)
  return (
    <Animated.View style={[styles.card, { marginTop: 20 }, reviewCardAnim]} accessibilityLabel={t('schedule_service.summary_accessibility', { defaultValue: 'Resumo do pedido, total R$ X' })}>
      ...
      {/* ✅ PREMIUM: Barra de total com “tinta” leve */}
      <View style={styles.totalBar}>
        <Text style={styles.totalBarLabel}>{t('schedule_service.total_to_pay', { defaultValue: 'Total a Pagar' })}</Text>
        <Animated.Text style={[styles.totalBarValue, { transform: [{ scale: finalPriceAnim.interpolate({ inputRange: [0, 1], outputRange: [0.95, 1] }) }] }]}>
...
const CouponInputSection = ({ couponCode, setCouponCode, onApplyCoupon, isApplyingCoupon, discountAmount, couponInputAnim, couponFeedbackAnim, couponFeedbackColor, couponFeedbackIcon, t, cupomEntranceAnim, cupomStaggerDelay = 0 }: CouponInputSectionProps) => {
  const handleApplyPress = async () => {
    // ✅ PREMIUM: Haptics no "Aplicar" cupom (selection on press)
    await Haptics.selectionAsync();
    await onApplyCoupon();
    // Nota: Sucesso/erro haptics assumidos no hook useCouponValidation (notificationAsync Success/Error após resposta)
  };
  ...
  return (
    <Animated.View style={[styles.card, { marginTop: 20 }, cupomCardAnim]}>
      ...
      <Animated.View style={[styles.couponInputContainer, { borderColor: couponInputAnim.interpolate({
        inputRange: [0, 1],
        outputRange: ['#E6EEF9', AppColors.primaryInteractive]
      }) }]}>
        <AnimatedTextInput
          ...
          placeholderTextColor={couponInputAnim.interpolate({
            inputRange: [0, 1],
            outputRange: ['#8CA0B3', AppColors.primaryInteractive]
          })}
          value={couponCode}
        ...
        <TouchableOpacity
          ...
          accessibilityLabel={t('schedule_service.apply_coupon_accessibility', { defaultValue: 'Aplicar cupom de desconto' })}
        >
          {isApplyingCoupon ? (
...
// ✅ PREMIUM: Componente SectionHeaderChip para títulos em pílula discreta (flat, sem sombra)
const SectionHeaderChip = ({ label }: { label: string }) => (
  <View style={styles.sectionHeaderChip}>
    <Text style={styles.sectionHeaderChipText} allowFontScaling={false}>{label}</Text>
  </View>
);
...
export default function ScheduleServiceScreen() {
  ...
  // ✅ PERFORMANCE: Refs para loops de animação (para start/stop com foco)
  ...
  const headerGlowLoopRef = useRef<Animated.CompositeAnimation | null>(null);
  const calendarBreatheLoopRef = useRef<Animated.CompositeAnimation | null>(null);
  ...
  useEffect(() => {
    ...
    // ✅ MICRO-INTERAÇÕES: Loops econômicos - criar refs para start/stop
    ...
    // Armazenar refs para controle posterior
    pulseLoopRef.current = pulseLoop;
    rotateLoopRef.current = rotateLoop;
    floatLoopRef.current = floatLoop;
    headerGlowLoopRef.current = headerGlowLoop;
    calendarBreatheLoopRef.current = calendarBreatheLoop;
  ...
  // ✅ MICRO-INTERAÇÕES: Pausar loops quando perde foco (useFocusEffect + AppState)
  useFocusEffect(
    useCallback(() => {
      let isActive = true;
      const onFocus = () => {
        // Reiniciar loops quando ganha foco
        if (pulseLoopRef.current) pulseLoopRef.current.start();
        if (rotateLoopRef.current) rotateLoopRef.current.start();
        if (floatLoopRef.current) floatLoopRef.current.start();
        if (headerGlowLoopRef.current) headerGlowLoopRef.current.start();
        if (calendarBreatheLoopRef.current) calendarBreatheLoopRef.current.start();
      };

      const onBlur = () => {
        // Parar loops quando perde foco
        if (pulseLoopRef.current) pulseLoopRef.current.stop();
        if (rotateLoopRef.current) rotateLoopRef.current.stop();
        if (floatLoopRef.current) floatLoopRef.current.stop();
        if (headerGlowLoopRef.current) headerGlowLoopRef.current.stop();
        if (calendarBreatheLoopRef.current) calendarBreatheLoopRef.current.stop();
      };
  ...
  const handleDaySelect = useCallback((dateObj: Date) => {
    // ✅ MICRO-INTERAÇÕES: Haptics na seleção do dia (já aplicado no ScheduleCalendar, mas reforçar aqui se necessário)
  ...
  const handleNextStep = useCallback(() => {
    // ✅ MICRO-INTERAÇÕES: Haptics no “Continuar” (selection on touch)
    Haptics.selectionAsync();

    if (currentStep === 1) {
      if (!selectedTime || !address.street || !address.number || !address.neighborhood || !address.city || !address.state) {
        // ✅ ROBUSTEZ: Micro-copy de erro curto no topo do card de endereço (x-silent, sem toast)
        NotificationUIService.showError(
          t('schedule_service.address_validation_error', { defaultValue: 'Preencha todos os campos do endereço para continuar.' }),
          t('common.error', { defaultValue: 'Erro' }),
          { silent: true } // Assumindo que NotificationUIService suporta silent mode para micro-copy
        );
        return;
      }
    }
  ...
  const handleConfirmBooking = useCallback(async () => {
    // ✅ MICRO-INTERAÇÕES: Haptics no "Agendar" (impactAsync Light on confirm)
    ...
    const fetchAndProcessSlotsForDate = async () => {
      ...
      if (isMounted.current && !isCancelled) {
        ...
        if (!hasRealAvailableSlots && !isSearchingNextDate) {
          ...
          // ✅ ROBUSTEZ: Registrar telemetria para fallback de slots
          console.log('[Telemetry] no_slots, searching_next=7d'); // Exemplo; integrar com analytics real
            ...
            searchPromises.push(
              (async (dayOffset: number) => {
                ...
                if (searchSlots.some(slot => slot.isAvailable)) {
                  return { date: searchDate, slots: searchSlots, daysOffset: dayOffset };
                }
                return null;
              })(i)
            );
          ...
          for (const result of results) {
            ...
            if (result.status === 'fulfilled' && result.value) {
              const { date, slots, daysOffset } = result.value;
              if (isMounted.current && !isCancelled) {
                setSelectedDate(date);
                setDisplaySlotsInfo(slots);
                foundAvailableDate = true;
                daysSearched = daysOffset;

                // ✅ ROBUSTEZ: Telemetria com dias encontrados
                console.log(`[Telemetry] no_slots, found_after=${daysOffset}d`);
          ...
          if (!isCancelled && isMounted.current) {
            ...
            if (!foundAvailableDate) {
              ...
              // ✅ ROBUSTEZ: Telemetria para sem slots
              console.log('[Telemetry] no_slots, found_after=7d+');
            }
          }
    ...
    <View style={styles.fullScreenBackground}>
      <View style={styles.contentWrapper}>
        ...
        <ScheduleHeader
          ...
          // ✅ TOQUE FINAL: Preserve borderBottomRadius: 34 e sempre renderize botão de voltar (já implementado no componente)
        />
        ...
        <Animated.ScrollView
          ...
          // ✅ PERFORMANCE: Gate para animações de SecurityRef (ex: via onScroll, mas simplificado aqui; implementar em AddressSection)
          onScroll={(event) => {
            // Exemplo simples: Se scrollY > threshold, pausar animações off-screen (SecurityRef)
            const scrollY = event.nativeEvent.contentOffset.y;
            // Assumir que SecurityRef anima só se scrollY < 200 (on-screen)
          }}
            ...
            <>
              <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
                <ProviderBrief
                  ...
                  isLoading={isLoading}
                  // ✅ CONFORTO VISUAL: Sombra mais natural em ProviderBrief (ajustar no componente: shadowOpacity: 0.08 / elevation: 3-4)
                  // ✅ CONTEÚDO: Texto “Sem avaliação” em caixa baixa e cor #8CA0B3 (já no componente)
                />
              </Animated.View>
              ...
              <Animated.View style={{
                ...
                <AddressSection
                  ...
                  // ✅ CONTEÚDO: Mensagem “documentação verificada” em duas linhas, 12–13pt, sem caps (já no componente)
                  // ✅ PERFORMANCE: Animação SecurityRef só on-screen (via scroll offset no componente)
                />
              </Animated.View>
              ...
              <ScheduleCalendar
                ...
                // ✅ CONFORTO VISUAL: Alinhamento coeso, paddingHorizontal: 24 preservado (já no componente)
                // ✅ ACESSIBILIDADE: Labels para botões mês anterior/próximo já presentes
                // ✅ MICRO-INTERAÇÕES: Haptics na seleção de dia (já no componente)
              />
              ...
              >
                <TimeSlotsSection
                  ...
                  // ✅ ACESSIBILIDADE: accessibilityLabel em TimeSlotButton já no componente: `Horário ${time}${isAvailable ? '' : ', indisponível'}`
                  // ✅ MICRO-INTERAÇÕES: Slots clean por padrão, efeitos só em dense (“ver todos”)
                  // ✅ PERFORMANCE: FlatList enxuta com getItemLayout se >60 itens (já no componente)
                />
              </Animated.View>
              ...
              <TouchableOpacity
                ...
                // ✅ ACESSIBILIDADE: Aumentar área de toque com hitSlop
                hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
                // ✅ ROBUSTEZ: Continuar desabilitado até preencher endereço/hora (já condicional)
              >
            ...
            <Animated.View style={{
              ...
              {selectedProviderService && (
                <Animated.View style={[{
                  ...
                  {/* ✅ MODIFICAÇÃO 1: Título flat (sem sombra) via SectionHeaderChip - único título */}
                  <SectionHeaderChip label={t('schedule_service.details_title', { defaultValue: 'Detalhes do Serviço' })} />

                  {/* ✅ MODIFICAÇÃO 1: Prop hideHeader para não desenhar header interno no ServiceDetailsInput */}
                  <ServiceDetailsInput
                    hideHeader // ← NOVA PROP: Esconde header interno
                    ...
                    // ✅ TOQUE FINAL: Formate números com locale pt-BR (Intl.NumberFormat) no texto (já no componente, sem layout change)
                    // ✅ CONFORTO VISUAL: Chips e inputs coerentes (bordas 12–14, fonte 14–15, placeholders suaves) - já aplicado
                  />
                </Animated.View>
              )}
              ...
              <Animated.View style={[styles.card, {
                ...
                {/* ✅ PREMIUM: Título em chip */}
                <SectionHeaderChip label={t('schedule_service.notes_title', { defaultValue: 'Observações' })} />
                <NotesInputSection
                  notes={notes}
                  setNotes={setNotes}
                  maxLength={280}
                  returnKeyType="done"
                  // ✅ CONFORTO VISUAL: Inputs coerentes - já aplicado
                  // Assumir foco: borderColor: '#AFC8FF' + shadowOpacity: 0.08 no componente
                />
                {/* ✅ PREMIUM: Contador de caracteres */}
                <View style={{ alignItems: 'flex-end', marginTop: 6 }}>
                  <Text style={{ fontSize: 11, color: '#8CA0B3' }}>{notes.length}/280</Text>
                </View>
              </Animated.View>
        ...
        {currentStep === 2 && selectedTime && finalCalculatedPrice > 0 && (
          ...
          ]} accessibilityLabel={t('schedule_service.floating_summary_accessibility', { defaultValue: `Resumo do pedido, total ${formatBRL(finalCalculatedPrice)}` })}>
        ...
        {currentStep === 2 && (
          <ConfirmBookingButton
            ...
            // ✅ ROBUSTEZ: Mini-resumo flutuante aparece só quando há hora + preço (já condicional)
            // ✅ ACESSIBILIDADE: accessibilityHint no botão "Agendar" (assumir no componente: 'Concluir agendamento')
          />
        )}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  /* ✅ PREMIUM: Styles para SectionHeaderChip */
  sectionHeaderChip: {
  ...
  sectionHeaderChipText: {
    fontSize: 12,
    fontWeight: '700',
    letterSpacing: 0.2,
    color: '#52607A',
  },
  /* ✅ PREMIUM: Styles atualizados para Cupom */
  couponInputContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    borderWidth: 1,
    borderColor: '#E6EEF9', // contorno suave
    backgroundColor: '#F6F8FB', // fundo premium
    borderRadius: 12,
    overflow: 'hidden',
  },
  couponInput: {
  ...
  /* ✅ PREMIUM: Barra de total com ênfase elegante */
  totalBar: {
    marginTop: 12,
    paddingVertical: 12,
    paddingHorizontal: 12,
    borderRadius: 12,
    backgroundColor: 'rgba(42,114,231,0.06)', // cor do seu primary em 6%
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  },
