...
export default function ServiceDetailsInput(props: Props) {
  ...
  const computedTotal = totalPrice ?? (pricePerHour
    ? pricePerHour * Math.ceil(effectiveDuration / 60)
    : 0);

  return (
    <View style={styles.container}>
      {!hideHeader && <Text style={styles.legacyTitle}>Detalhes do Serviço</Text>}
      ...
      <View style={styles.inputGroup}>
        ...
        <Text style={styles.priceText}>
          Total estimado: R$ {computedTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
        </Text>
      </View>
    </View>
  );
